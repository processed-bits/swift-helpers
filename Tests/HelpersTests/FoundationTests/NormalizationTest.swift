// NormalizationTest.swift, 17.04.2024-30.01.2025.
...
struct NormalizationTest {
	...
	/// The tested URL.
	let url: URL
	private var isAbsolute: Bool { url == url.absoluteURL }
	...
	private var inputURLComment: Comment { "Input URL: \(url.description)" }
	...
	@discardableResult func assertAbsoluteString(sourceLocation: SourceLocation = #_sourceLocation) throws -> Self {
		...
		withKnownIssue(isIntermittent: true, sourceLocation: sourceLocation) {
			#expect(result == expected, inputURLComment, sourceLocation: sourceLocation)
		}
		return self
	}
	...
	@discardableResult func assertStandardized(sourceLocation: SourceLocation = #_sourceLocation) throws -> Self {
		let standardizedURL = if resolve {
	...
	@discardableResult func assertLexicallyNormalizedFilePath(sourceLocation: SourceLocation = #_sourceLocation) throws -> Self {
		guard url.isFileURL else {
			Issue.record("Not a file URL, remove test assertion.", sourceLocation: sourceLocation)
			return self
		}

		// If the URL is being resolved, create a `FilePath` using the URL. Otherwise, using the URL's relative path.
		let filePath = if resolve {
			// `FilePath` initializer always resolves a URL with a relative path against its base URL.
			try #require(FilePath(url), sourceLocation: sourceLocation)
		} else {
