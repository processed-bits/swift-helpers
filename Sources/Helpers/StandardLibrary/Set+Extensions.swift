// Set+Extensions.swift, 28.01.2022-26.11.2022.
...
public extension Set {
	...
	init(from string: String, separator: Character, maxSplits: Int = .max, omittingEmptySubsequences: Bool = true, trimCharacters: CharacterSet? = .whitespaces) where Element == String {
		...
		let strings: [String]
		if let trimCharacters {
			strings = substrings.map { $0.trimmingCharacters(in: trimCharacters) }
		} else {
			strings = substrings.map { String($0) }
		}
		self.init(strings)
	}
