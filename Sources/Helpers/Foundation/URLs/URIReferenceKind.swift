// URIReferenceKind.swift, 27.11.2024-24.01.2025.
...
public enum URIReferenceKind: Equatable, Hashable {
	...
	///
	/// - Note: This initializer is not meant to be invoked directly. Use respective `URL`'s ``Foundation/URL/referenceKind`` and `URLComponents`' ``Helpers/Foundation/URLComponents/referenceKind`` properties, which in turn use this initializer, to get a URI reference kind.
	///
	/// This initializer uses autoclosures for most of its parameters for improved efficiency.
	///
	/// See [Default Implementation](<doc:URIReferenceKind#Default-Implementation>) for more information.
	public init(
		scheme: String?,
		host hostProvider: @autoclosure () -> String?,
		path pathProvider: @autoclosure () -> String,
		query queryProvider: @autoclosure () -> String?,
		fragment fragmentProvider: @autoclosure () -> String?
	) {
		lazy var host = hostProvider()
		lazy var path = pathProvider()
		lazy var query = queryProvider()
		lazy var fragment = fragmentProvider()
