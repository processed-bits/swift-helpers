...
This article summarizes parsing peculiarities of [`URL`](https://developer.apple.com/documentation/foundation/url) and [`URLComponents`](https://developer.apple.com/documentation/foundation/urlcomponents) from the Foundation framework, and related extensions, that are provided in this package.
...
- relative file paths get current working directory as a base URL, if no base is specified;
- an empty file path is initialized as `./`;
- `./` path prefixes are removed when followed by other path components;
- `./` or `/.` path suffixes are removed when preceded by other path components.
...
Type | Member
--- | ---
`URL` | ``Foundation/URL/isValidBaseURL``
`URL` | ``Foundation/URL/asBaseURL``
`URLComponents` | ``Foundation/URLComponents/isValidBaseURL``
...
A plain colon is used to delimit user and password components in a string form. It is not among allowed (non-percent-encoded) characters in the [`urlUserAllowed`](https://developer.apple.com/documentation/foundation/characterset/1780246-urluserallowed) and [`urlPasswordAllowed`](https://developer.apple.com/documentation/foundation/characterset/1780126-urlpasswordallowed) character sets.  Despite that, `URL` and `URLComponents` string initializers, as well as `URLComponents`' `user` and `password` property setters do not percent-encode a colon.
...
Related normalization methods, that are provided by this package, leave any colons (both percent-encoded and not) in the user and password components unchanged.
