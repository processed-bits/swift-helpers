...
The default initializer implementation (which is used by respective `URL`'s ``Foundation/URL/referenceKind`` and `URLComponents`' ``Helpers/Foundation/URLComponents/referenceKind`` properties) uses a “first-match-wins” algorithm. 
